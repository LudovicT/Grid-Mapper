...
namespace ConsoleApplication5
{
	class RawSocketPing
	{
		...
		public RawSocketPing( AddressFamily af, int ttlValue, int payloadSize, int sendCount, ushort idValue )
			: this()
		{
			pingFamily = af;
			pingTtl = ttlValue;
		...
		{
			get { return destEndPoint.Address; }
			set { destEndPoint = new IPEndPoint( value, 0 ); }
		}
		...
		public void InitializeSocket()
		{
			...
			else if( destEndPoint.AddressFamily == AddressFamily.InterNetworkV6 )
			{
				// Create the raw socket
				Console.WriteLine( "Creating raw socket using Socket()..." );
				pingSocket = new Socket( destEndPoint.AddressFamily, SocketType.Raw, (ProtocolType)58 /* ICMPv6 protocol value */);
				...
				pingSocket.SetSocketOption( SocketOptionLevel.IPv6, SocketOptionName.IpTimeToLive, pingTtl );

				// Allocate the buffer used to receive the response
				Console.WriteLine( "Allocating buffer for the received response..." );
				receiveBuffer = new byte[Ipv6Header.Ipv6HeaderLength + Icmpv6Header.Icmpv6HeaderLength + Icmpv6EchoRequest.Icmpv6EchoRequestLength + pingPayloadLength];
		...
		public void DoPing()
		{
			...
			try
			{
				...
				pingSocket.BeginReceiveFrom( receiveBuffer, 0, receiveBuffer.Length, SocketFlags.None, ref castResponseEndPoint, receiveCallback, this );
